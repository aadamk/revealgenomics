...
The initialization steps are provided below. Currently, we need to log in as the root database user to load data to the pan-study repository. 
```{r init}
library(scidb4gh)
gh_connect(username = 'secure_user', 
...
```{r project-bloc3}
project_id = register_project(df = df_project, namespace = "collaboration")
print(project_id)
```
...
RNAQuantificationSets hold information for data processing pipelines within a study (more details are provided at this GA4GH [link](http://ga4gh-schemas.readthedocs.io/en/latest/api/rna_quantifications.html)). Below, we register two pipelines corresponding to two RNASeq data processing algorithms:
```{r rqset-bloc1}
mandatory_fields()[['RNAQUANTIFICATIONSET']]
```
...
register_rnaquantificationset(pipeline_df, only_test = TRUE)
```

Next we register the pipelines.
```{r rqset_bloc3}
pipeline_record = register_rnaquantificationset(df = pipeline_df)
...
```{r data_bloc1, eval = FALSE, include = TRUE}
# GENE_ID Jake_Sample1  Jake_Sample2  Linda_Sample1 ...
# ENSG00000254029 48.8129 0.0 0.0 ...
# ENSG00000250939 0.0 0.0 2.98953 ...
# ENSG00000225275 0.0 0.0 0.0 ...
```

These matrix files can sometimes be very large, and we provide an API to directly load the data into SciDB:

```{r reg-exprn-set, eval = TRUE, include = TRUE}
# Choose the first pipeline for 'Salmon  Gene counts'
rnaquantificationset_id = pipeline_record[1, ]$rnaquantificationset_id

# register expression data 
register_expression_matrix( filepath = system.file("extdata", "rnaseq.txt", 
                                                   package = "scidb4gh"),
...
rnaquantificationset = get_rnaquantificationsets(rnaquantificationset_id = 
                                                   rnaquantificationset_id)
eset = search_rnaquantification(rnaquantificationset = rnaquantificationset)
print(eset)
exprs(eset)
```
