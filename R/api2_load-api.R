...
#' 
#' @param biosample_names \code{sample_name} column from rows of Pipeline sheet corresponding to current pipeline
...
api_register_measurements = function(
  ...
  candidate_biosample_ids = bios_df_ref[m1$target_matched_idx, ]$biosample_id
  candidate_biosample_names = bios_df_ref[m1$target_matched_idx, ]$name
  if (length(unique(candidate_biosample_ids)) 
      != length(candidate_biosample_ids)) {
    cat("**ATTENTION**: More than one entry for following samples at given pipeline + filter combination:",
        pretty_print(candidate_biosample_names[which(duplicated(candidate_biosample_names))]), 
        "\n")
  }
  msmt_df = data.frame(
    ...
    biosample_id         = unique(candidate_biosample_ids))
